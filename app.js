...
$("#admin-list").addEventListener("click", async (event) => { const edit = event.target.closest("[data-edit]"); const remove = event.target.closest("[data-delete]"); if (edit) { const article = state.adminArticles.find((item) => String(item.id) === edit.dataset.edit); if (article) { $("#edit-id").value = article.id; $("#edit-course").value = article.course; $("#edit-title").value = article.title; $("#edit-description").value = article.description; setEditorContent(article.content); $("#edit-status").value = article.status; $("#edit-order").value = article.sort_order; } } if (remove && window.confirm("Удалить статью безвозвратно? Связанный прогресс также будет удалён.")) { const { error } = await db.from("articles").delete().eq("id", remove.dataset.delete); if (error) { message("#admin-message", error.message); return; } message("#admin-message", "Статья удалена.", true); await renderAdminList(); await loadArticles(); } });
