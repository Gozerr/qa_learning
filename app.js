...
function syncEditor() { $("#edit-content").value = window.DOMPurify.sanitize($("#edit-content-editor").innerHTML, { USE_PROFILES: { html: true } }); }
function resetEditor() { $("#edit-content-editor").innerHTML = ""; $("#edit-content").value = ""; }
function focusEditor() { $("#edit-content-editor").focus(); }
document.querySelectorAll("[data-command]").forEach((button) => button.addEventListener("click", () => { focusEditor(); const command = button.dataset.command; if (command === "formatBlock") document.execCommand(command, false, `<${button.dataset.value}>`); else document.execCommand(command, false); syncEditor(); }));
document.querySelectorAll("[data-editor-action]").forEach((button) => button.addEventListener("click", () => { focusEditor(); const action = button.dataset.editorAction; const value = window.prompt(action === "image" ? "URL изображения" : "URL ссылки"); if (!value) return; if (action === "image") document.execCommand("insertImage", false, value.trim()); else document.execCommand("createLink", false, value.trim()); syncEditor(); }));
$("#edit-content-editor").addEventListener("input", syncEditor);
$("#article-form").addEventListener("submit", async (event) => { event.preventDefault(); syncEditor(); const id = $("#edit-id").value; const content = $("#edit-content").value.trim(); if (!content) { message("#admin-message", "Добавьте содержание статьи."); return; } const payload = { course: $("#edit-course").value, title: $("#edit-title").value.trim(), description: $("#edit-description").value.trim(), content, status: $("#edit-status").value, sort_order: Number($("#edit-order").value || 0) }; const result = id ? await db.from("articles").update(payload).eq("id", id) : await db.from("articles").insert(payload); if (result.error) { message("#admin-message", result.error.message); return; } message("#admin-message", "Статья сохранена.", true); event.target.reset(); $("#edit-id").value = ""; resetEditor(); await renderAdminList(); await loadArticles(); });
$("#admin-list").addEventListener("click", async (event) => { const edit = event.target.closest("[data-edit]"); const remove = event.target.closest("[data-delete]"); if (edit) { const article = state.adminArticles.find((item) => String(item.id) === edit.dataset.edit); if (article) { $("#edit-id").value = article.id; $("#edit-course").value = article.course; $("#edit-title").value = article.title; $("#edit-description").value = article.description; $("#edit-content-editor").innerHTML = window.DOMPurify.sanitize(article.content, { USE_PROFILES: { html: true } }); syncEditor(); $("#edit-status").value = article.status; $("#edit-order").value = article.sort_order; $("#edit-content-editor").focus(); } } if (remove && window.confirm("Удалить статью безвозвратно? Связанный прогресс также будет удалён.")) { const { error } = await db.from("articles").delete().eq("id", remove.dataset.delete); if (error) { message("#admin-message", error.message); return; } message("#admin-message", "Статья удалена.", true); await renderAdminList(); await loadArticles(); } });
