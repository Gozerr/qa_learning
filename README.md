...
- Изображения загружаются в приватный Storage с проверкой MIME-типа и лимитом 5 МБ; для чтения используются временные signed URL.
- HTML статьи очищается DOMPurify перед отображением в браузере.
- Для статей хранятся версии, метаданные курса/уровня и audit log действий администратора.
- Практика и инструменты хранятся в отдельной таблице ресурсов и редактируются из CMS.
...
Кнопка шаблона помогает создать структуру полноценной главы: определение, объяснение,
примеры, рабочий QA-сценарий, ошибки, практика, самопроверка, тезисы и источники.
В разделе ресурсов администратор может менять название, описание, категорию, URL и
порядок карточек, а также добавлять и удалять блоки практики и инструментов.
